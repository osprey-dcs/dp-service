...
/**
 * This integration test covers the use of protobuf StringColumns in the MLDP APIs.
 */
public class StringColumnIT extends GrpcIntegrationTestBase {
    ...
    /**
     * This test case provides full MLDP API coverage for use of StringColumns.
     * Registers a provider, which is required before ingesting data.
     * Creates a request with StringColumns and ingests data.
     * Queries the ingested data and verifies the query result includes the StringColumns.
     * Runs a subscription for the PVs in the StringColumns and ingests additional data and verifies subscription receives data.
     * Runs a data event subscription for the PVs in the StringColumns and ingests additional data and verifies subscription receives events.
     */
    @Test
    public void testStringColumnFullApiCoverage() throws Exception {

        // register provider
        final String providerName = "string-column-integration-test-provider";
        final String providerId = ingestionServiceWrapper.registerProvider(providerName, null);

        // create test data using StringColumns
        ...
        int numSamples = 5;
        final List<String> pvNames = List.of("test_pv_string_01", "test_pv_string_02");
        {
            // create StringColumns for ingestion
            List<StringColumn> stringColumns = new ArrayList<>();
            for (String pvName : pvNames) {
                List<String> values = new ArrayList<>();
                for (int i = 0; i < numSamples; i++) {
                    values.add(i % 2 == 0 ? "active" : "idle"); // "active", "idle", "active", "idle", "active"
                }
        ...
        int expectedEventResponseCount = 0;
        {
            ...
            SubscribeDataEventResponse.Event event;
            {
                ...
                PvConditionTrigger trigger = PvConditionTrigger.newBuilder()
                        ...
                        .setValue(DataValue.newBuilder().setStringValue("running").build())
                        .build();
                ...
                final DataValue eventDataValue = DataValue.newBuilder().setStringValue("running").build();
        ...
        StringColumn subscriptionColumn;
        {
            // positive unary ingestion test for StringColumn
            ...
            // specify explicit StringColumn data
            final List<StringColumn> stringColumns = new ArrayList<>();
            StringColumn.Builder stringColumnBuilder = StringColumn.newBuilder();
            stringColumnBuilder.setName(pvNames.get(0));
            stringColumnBuilder.addValues("running");
            stringColumnBuilder.addValues("stopped");
            subscriptionColumn = stringColumnBuilder.build();
            stringColumns.add(subscriptionColumn);
