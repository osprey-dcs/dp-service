...
/**
 * This integration test covers the use of protobuf BoolColumns in the MLDP APIs.
 */
public class BoolColumnIT extends GrpcIntegrationTestBase {
    ...
    /**
     * This test case provides full MLDP API coverage for use of BoolColumns.
     * Registers a provider, which is required before ingesting data.
     * Creates a request with BoolColumns and ingests data.
     * Queries the ingested data and verifies the query result includes the BoolColumns.
     * Runs a subscription for the PVs in the BoolColumns and ingests additional data and verifies subscription receives data.
     * Runs a data event subscription for the PVs in the BoolColumns and ingests additional data and verifies subscription receives events.
     */
    @Test
    public void testBoolColumnFullApiCoverage() throws Exception {

        // register provider
        final String providerName = "bool-column-integration-test-provider";
        final String providerId = ingestionServiceWrapper.registerProvider(providerName, null);

        // create test data using BoolColumns
        ...
        int numSamples = 5;
        final List<String> pvNames = List.of("test_pv_bool_01", "test_pv_bool_02");
        {
            // create BoolColumns for ingestion
            List<BoolColumn> boolColumns = new ArrayList<>();
            for (String pvName : pvNames) {
                List<Boolean> values = new ArrayList<>();
                for (int i = 0; i < numSamples; i++) {
                    values.add(i % 2 == 0); // true, false, true, false, true
                }
                BoolColumn boolColumn = BoolColumn.newBuilder()
                        .setName(pvName)
                        .addAllValues(values)
                        .build();
                boolColumns.add(boolColumn);
            }
        ...
        int expectedEventResponseCount = 0;
        {
            ...
            SubscribeDataEventResponse.Event event;
            {
                ...
                PvConditionTrigger trigger = PvConditionTrigger.newBuilder()
                        ...
                        .setValue(DataValue.newBuilder().setBooleanValue(true).build())
                        .build();
                ...
                final DataValue eventDataValue = DataValue.newBuilder().setBooleanValue(true).build();
        ...
        BoolColumn subscriptionColumn;
        {
            // positive unary ingestion test for BoolColumn
            ...
            // specify explicit BoolColumn data
            final List<BoolColumn> boolColumns = new ArrayList<>();
            BoolColumn.Builder boolColumnBuilder = BoolColumn.newBuilder();
            boolColumnBuilder.setName(pvNames.get(0));
            boolColumnBuilder.addValues(true);
            boolColumnBuilder.addValues(false);
            subscriptionColumn = boolColumnBuilder.build();
            boolColumns.add(subscriptionColumn);
            ...
            subscriptionRequestParams.setBoolColumnList(boolColumns);
