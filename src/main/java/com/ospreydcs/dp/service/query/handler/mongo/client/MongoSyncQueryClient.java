...
//        aggregateIterable.forEach(bucketDocument -> {System.out.println(bucketDocument.toString());});
    ...
    public MongoCursor<AnnotationDocument> executeQueryAnnotations(QueryAnnotationsRequest request) {
        ...
        Bson ownerFilter = Filters.exists(BsonConstants.BSON_KEY_ANNOTATION_ID);
        if (! ownerId.isBlank()) {
            ownerFilter = Filters.eq(BsonConstants.BSON_KEY_ANNOTATION_OWNER_ID, ownerId);
        }

        // TODO: modify to use list of search criteria
...
//        logger.debug("executing queryAnnotations filter: " + queryFilter.toString());
        ...
        // TODO: modify filter to use list of search criteria
